...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-background bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(124,58,237,0.15),rgba(255,255,255,0))]">
      <div className="container py-8 space-y-10">
        
        {/* Hero Section */}
        <section className="glass-card grid gap-6 overflow-hidden rounded-3xl p-6 shadow-2xl md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div className="flex flex-col justify-center gap-5">
            <Badge className="w-fit gap-1.5 rounded-full px-3.5 py-1 text-xs font-semibold bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-colors">
              <Sparkles className="h-3.5 w-3.5" /> Biblioteca premium de manhwas
            </Badge>
            <div className="space-y-3">
              <h1 className="max-w-3xl text-3xl font-black tracking-tight sm:text-5xl leading-tight">
                Encontre sua próxima leitura favorita em segundos.
              </h1>
              <p className="max-w-2xl text-muted-foreground sm:text-lg">
                Explore destaques, rankings, capítulos recentes e novas opções de filtros para navegar pelo catálogo com mais conforto.
              </p>
            </div>
            <div className="relative max-w-2xl">
              ...
              <Input 
                ...
                className="h-13 rounded-2xl border-border bg-background/50 pl-12 text-base shadow-inner transition-all focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" 
                value={search} 
        ...
        <section className="glass-card space-y-4 rounded-2xl p-5 shadow-sm">
        ...
        {isLoading ? (
          ...
          <HomeSections featured={featured} trending={trending} recent={recent} completed={completed} all={library} />
        )}
      </div>
    </div>
  );
    ...
    <section>
      ...
      <ScrollArea className="w-full whitespace-nowrap rounded-2xl">
        <div className="flex gap-4 pb-4">
          {manhwas.map((m) => (
            <div key={m.id} className={compact ? "w-[150px] shrink-0" : "w-[175px] sm:w-[195px] shrink-0"}>
...
function ManhwaCard({ manhwa, showViews }: { manhwa: Manhwa; showViews?: boolean }) {
  return (
    <Link 
      to={`/manhwa/${manhwa.id}`} 
      className="glow-hover group block overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm hover:border-primary/50"
    >
      <div className="aspect-[3/4] overflow-hidden bg-muted relative">
        ...
        <div className="gradient-overlay absolute inset-0 flex items-end p-3 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <p className="line-clamp-3 text-xs leading-relaxed text-slate-200">
            {manhwa.synopsis || "Toque para ver detalhes e capítulos disponíveis."}
          </p>
        </div>

        {manhwa.status === "ongoing" && (
          <Badge variant="destructive" className="absolute right-2 top-2 text-[10px] font-bold shadow-md">
            LANÇANDO
          </Badge>
        )}
      </div>
      ...
      <div className="space-y-2 p-3">
        ...
        <div className="flex flex-wrap gap-1">
          {manhwa.genres?.slice(0, 2).map((g) => (
            <Badge key={g} variant="secondary" className="rounded-md px-1.5 py-0 text-[10px] bg-secondary/80 text-secondary-foreground">
              {g}
            </Badge>
          ))}
        </div>
...
function RankingCard({ manhwa, rank }: { manhwa: Manhwa; rank: number }) {
  return (
    <Link 
      to={`/manhwa/${manhwa.id}`} 
      className="group flex items-center gap-4 rounded-2xl border border-border/60 bg-card/60 p-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-card hover:shadow-lg"
    >
      <span className={`text-2xl font-black w-8 text-center shrink-0 ${rank <= 3 ? "text-primary" : "text-muted-foreground/60"}`}>
        {rank}
      </span>
      <div className="h-16 w-12 overflow-hidden rounded-xl bg-muted shrink-0 shadow-sm">
        {manhwa.cover_url ? (
          <img src={manhwa.cover_url} alt={manhwa.title} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[10px] text-muted-foreground">—</div>
        )}
      </div>
      <div className="min-w-0 flex-1">
        <h3 className="truncate text-sm font-semibold transition-colors group-hover:text-primary">{manhwa.title}</h3>
...
function HeroPreview({ manhwa }: { manhwa?: Manhwa }) {
  if (!manhwa) return <div className="hidden rounded-3xl border border-dashed border-border bg-muted/20 p-8 text-center text-muted-foreground md:flex md:items-center md:justify-center">Adicione manhwas para preencher o destaque.</div>;
  
  return (
    <Link to={`/manhwa/${manhwa.id}`} className="group relative min-h-[340px] overflow-hidden rounded-3xl border border-border/80 bg-muted shadow-lg">
      <img src={manhwa.cover_url || "/placeholder.svg"} alt={manhwa.title} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <div className="absolute inset-x-0 bottom-0 space-y-3 p-6 text-white">
        ...
        <h2 className="text-2xl font-black drop-shadow-md sm:text-3xl">{manhwa.title}</h2>
        <p className="line-clamp-2 text-xs text-slate-300 sm:text-sm drop-shadow">{manhwa.synopsis || "Veja detalhes, capítulos e informações da obra."}</p>
        <Button size="sm" className="rounded-full shadow-lg transition-transform group-hover:scale-105">
          Começar a ler
        </Button>
      </div>
    </Link>
...
function StatCard({ icon: Icon, label, value }: { icon: typeof BookOpen; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-border/60 bg-background/40 p-3.5 backdrop-blur-sm">
